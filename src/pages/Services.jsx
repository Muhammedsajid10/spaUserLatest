...
const ServiceModal = ({ selectedService, selectedOption, setSelectedOption, handleAddToBooking, handleCloseModal }) => (
...
function Services() {
  // --- START: Core State and Business Logic (Untouched) ---
...
  }, []);
  
  // --- START: Refactored UI/UX Logic ---
  const headerRef = useRef(null);
  const [mainContentPaddingTop, setMainContentPaddingTop] = useState(0);
...
    if (currentNav) {
      currentNav.addEventListener('scroll', handleNavScroll);
      const resizeObserver = new ResizeObserver(handleNavScroll);
      resizeObserver.observe(currentNav);
      handleNavScroll();
      return () => {
        currentNav.removeEventListener('scroll', handleNavScroll);
        resizeObserver.unobserve(currentNav);
      };
    }
  }, [services]);
...
        setMainContentPaddingTop(headerRef.current.offsetHeight);
      }
    };
