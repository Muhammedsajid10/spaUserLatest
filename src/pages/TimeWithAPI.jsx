...
const Time = (props) => {
  ...
  const fetchDateData = async (date) => {
    ...
    // 1) fetch employees (try date-specific endpoint, then weekly, then all)
    let employees = [];
    try {
      if (employeesAPI && typeof employeesAPI.getEmployeesByDate === 'function') {
        const resp = await employeesAPI.getEmployeesByDate(formattedDate);
        employees = resp?.data?.employees ?? resp?.employees ?? resp?.data ?? resp ?? [];
      } else if (employeesAPI && typeof employeesAPI.getEmployeesByWeekStart === 'function') {
        const d = new Date(date);
        const day = d.getDay();
        const diffToMon = (day === 0) ? 6 : day - 1;
        const weekStart = new Date(d);
        weekStart.setDate(d.getDate() - diffToMon);
        const weekStartISO = apiUtils.formatDate(weekStart);
        const resp = await employeesAPI.getEmployeesByWeekStart(weekStartISO);
        employees = resp?.data?.employees ?? resp?.employees ?? resp?.data ?? resp ?? [];
      } else if (employeesAPI && typeof employeesAPI.getAllEmployees === 'function') {
        const resp = await employeesAPI.getAllEmployees();
        employees = resp?.data?.employees ?? resp?.employees ?? resp?.data ?? resp ?? [];
      } else {
        employees = [];
      }
    } catch (err) {
      console.warn('[Time] failed to fetch employees for date', formattedDate, err);
      employees = [];
    }
    ...
    // 2) fetch bookings for the date (try date-specific endpoint then range)
    let bookings = [];
    try {
      if (bookingsAPI && typeof bookingsAPI.getUserBookings === 'function') {
        const resp = await bookingsAPI.getUserBookings(formattedDate);
        bookings = resp?.data?.bookings ?? resp?.bookings ?? resp?.data ?? resp ?? [];
      } else if (bookingsAPI && typeof bookingsAPI.getBookingsForRange === 'function') {
        const resp = await bookingsAPI.getBookingsForRange(formattedDate, formattedDate);
        bookings = resp?.data?.bookings ?? resp?.bookings ?? resp?.data ?? resp ?? [];
      } else if (bookingsAPI && typeof bookingsAPI.getBookings === 'function') {
        const resp = await bookingsAPI.getBookings({ date: formattedDate });
        bookings = resp?.data?.bookings ?? resp?.bookings ?? resp?.data ?? resp ?? [];
      } else {
        bookings = [];
      }
    } catch (err) {
      console.warn('[Time] failed to fetch bookings for date', formattedDate, err);
      bookings = [];
    }
