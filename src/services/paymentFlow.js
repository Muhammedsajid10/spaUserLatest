...
const createBookingIfNeeded = async () => {
  ...
  // Ensure appointmentDate shares the same calendar date as the first service start/end
  // Derive Y-M-D from the selected slot start; anchor to 12:00:00Z to avoid timezone rollovers
  const slotStart = bookingFlow.selectedTimeSlot?.startTime
    ? new Date(bookingFlow.selectedTimeSlot.startTime)
  ...
  const alignedAppointmentDate = `${year}-${month}-${day}T12:00:00.000Z`;
  ...
  const bookingPayload = {
    ...
    appointmentDate: alignedAppointmentDate,
    notes: '',
